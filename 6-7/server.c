...
struct sockaddr_in observer;
socklen_t observer_size;
int server_socket;
char observer_message[256];

void send_message_to_observer(char* message) {
    if (sendto(server_socket, message, strlen(message), 0, (struct sockaddr*)&observer, observer_size) == -1) {
...
int main(int argc, char* argv[]) {
    ...
    while ((num_clients < TOTAL_CLIENTS) || !num_observers) {
        char message[256];
        ...
        if (strcmp(message, "observer") == 0) {
            observer = client_addr;
            observer_size = addr_size;
